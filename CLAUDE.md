...
- **Phase 4 (2026-09-04 brief)** — `app/dashboard/(app)/quiet` (the quiet question: encrypted `quiet_reflections`, themes confirmed by the member, safety path = 988 care card + one private leader email via `lib/notify/templates.ts` safetyLeaderMail), `lib/dashboard/quiet-themes.ts` (theme words for leaders only when 3+ members wrote), `lib/notify/prayer.ts` (prayer wall pushes), `rhythmPush` in `lib/notify/scheduled.ts` (17:00 Denver, once per fortnight per member), `lib/dashboard/{bible,verses,themes,rhythm,questions}.ts` + `verses.json` (366 WEB verses, built once by `scripts/build-verses.mjs`) + `themes.json` (the ~30-theme lexicon; safety patterns are matched apart and never become a theme), `components/dashboard/{VerseCard,FromTheWord}.tsx`, `components/lead/{WhoIsFree,RefreshBusy}.tsx`; migration 020 adds the Scripture columns on events (context_notes is leader-only), member_prefs.share_themes / rhythm_nudged_at, and quiet_reflections (encrypted body). Availability rule: a member with a connected calendar and no weekly rows is free wherever not busy (`memberIsFree`).
